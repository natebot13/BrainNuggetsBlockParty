...
task main(){
	...
	if(nMotorEncoder[arm] > 0){
		...
		while(nNxtButtonPressed == -1){
			nVolume = 4;
			if(!bSoundActive){PlaySound(soundFastUpwardTones);}
		}
	}*/
...
*/
	...
	nMotorEncoder[x2] = 0;
	while(nMotorEncoder[x2] > -2500){
		polarDrive(x1,x2,y1,y2,75,offset-90,0);
	}
	stopPolarMot(x1,x2,y1,y2);

	nMotorEncoder[x2] = 0;
	while(nMotorEncoder[x2] > -4500){
		polarDrive(x1,x2,y1,y2,50,offset+0,0);
	}
	stopPolarMot(x1,x2,y1,y2);
